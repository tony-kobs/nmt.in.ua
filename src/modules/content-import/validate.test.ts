...
test("validateThemesDataset accepts a well-formed row", () => {
  const result = validateThemesDataset([
    row("themes row 2", { id: "1", name: "Algebra", description: "Basics", ord: "1" }),
  ]);
...
test("validateThemesDataset rejects unknown and missing fields", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("missing field(s): description")));
});
...
test("validateThemesDataset rejects an oversized name", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("name must be at most 100 characters")));
});
...
test("validateThemesDataset accepts ord = 0 (a valid ordinal position)", () => {
  const result = validateThemesDataset([
    row("themes row 2", { id: "1", name: "Algebra", description: "Basics", ord: "0" }),
  ]);
  assert.deepEqual(result.errors, []);
...
test("validateThemesDataset rejects a negative ord", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("ord must be a non-negative integer")));
});

test("validateThemesDataset rejects id = 0 (primary IDs must be positive)", () => {
  const result = validateThemesDataset([
    row("themes row 2", { id: "0", name: "Algebra", description: "Basics", ord: "1" }),
  ]);
  assert.ok(result.errors.some((e) => e.includes("id must be a positive integer")));
});
...
test("validateThemesDataset rejects an id one past the MySQL INT upper bound", () => {
  const result = validateThemesDataset([
    row("themes row 2", { id: "2147483648", name: "Algebra", description: "Basics", ord: "1" }),
  ]);
  assert.ok(result.errors.some((e) => e.includes("id must be between -2147483648 and 2147483647")));
});
...
test("validateThemeConnectionsDataset rejects a zero vertex_start (foreign references must be positive)", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("vertex_start must be a positive integer")));
});
...
test("validateThemeConnectionsDataset accepts JSON-typed integers directly", () => {
  ...
  assert.deepEqual(result.errors, []);
  assert.deepEqual(result.records, [{ id: 1, vertexStart: 2, vertexFinish: 3 }]);
});
...
test("validateQuizTasksDataset accepts a well-formed row", () => {
  const result = validateQuizTasksDataset([row("quizTasks row 2", validQuizTaskRaw())]);
  assert.deepEqual(result.errors, []);
...
test("validateQuizTasksDataset rejects right_answer_n outside 1..4", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("right_answer_n must be between 1 and 4")));
});
...
test("validateQuizTasksDataset rejects a non-integer right_answer_n", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("right_answer_n must be an integer")));
});
...
test("validateQuizTasksDataset rejects a negative theme_id (foreign reference must be positive)", () => {
  ...
  assert.ok(result.errors.some((e) => e.includes("theme_id must be a positive integer")));
});

test("validateQuizTasksDataset rejects an oversized answer", () => {
  const result = validateQuizTasksDataset([
    row("quizTasks row 2", validQuizTaskRaw({ answer_1: "x".repeat(51) })),
  ]);
  assert.ok(result.errors.some((e) => e.includes("answer_1 must be at most 50 characters")));
});
