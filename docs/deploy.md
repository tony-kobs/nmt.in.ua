...
WayForPay-ключі (`WAYFORPAY_MERCHANT_*`) на хості не потрібні для реєстрації викладача: `/register/teacher` створює акаунт одразу. Модуль і таблиця `teacher_payments` лишаються на майбутній еквайринг.
